...
0. **AUTO PUSH OBRIGATÓRIO (CRITICAL):** O agente DEVE SEMPRE, sem exceção, executar o envio para o repositório (`git add . && git commit -m "..." && git push`) de forma proativa **APÓS QUALQUER MODIFICAÇÃO** ou conclusão de tarefa no código. Não espere o usuário pedir para fazer o commit.
1. **GitHub Always On:** Sempre que uma alteração funcional ou correção de bug for concluída e validada localmente com sucesso, o agente DEVE imedatamente executar o envio para o GitHub para garantir a integração contínua (CI/CD) com o Coolify. Nunca encerrar uma sessão de sucesso sem o push.
