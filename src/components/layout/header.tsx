...
export function Header({ title }: { title?: string }) {
    ...
    return (
        <header className="h-[72px] border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 backdrop-blur-none px-8 flex items-center justify-between sticky top-0 z-20">
            <div className="flex items-center gap-4 flex-1">
                <h2 className="text-xl font-semibold text-slate-900 dark:text-white tracking-tight">{title || "Overview"}</h2>
                <div className="hidden md:flex relative w-64 ml-8 group">
                    ...
                    <input
                        className="w-full pl-10 pr-4 py-2 bg-slate-100/50 dark:bg-white/5 border border-transparent dark:border-white/5 rounded-lg text-sm focus:bg-white dark:focus:bg-white/10 focus:ring-2 focus:ring-primary/20 focus:border-primary/30 placeholder:text-slate-400 outline-none transition-all shadow-sm shadow-black/5"
            ...
            <div className="flex items-center gap-4">
                ...
                {action && ActionIcon && (
                    <>
                        ...
                        <Link href={action.href} className="bg-primary text-white px-5 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2.5 hover:bg-primary/90 transition-colors">
